...
export default async function DashboardPage() {
  ...
  const [[ordersCount], [downloadsCount], [reviewsCount], recentOrders] = await Promise.all([
    sql`SELECT COUNT(*)::int AS count FROM orders WHERE user_id=${user.id}`,
    sql`SELECT COUNT(*)::int AS count FROM orders WHERE user_id=${user.id} AND status IN ('paid','completed')`,
    sql`SELECT COUNT(*)::int AS count FROM reviews WHERE user_id=${user.id}`,
    sql`
      SELECT o.id, o.status, o.amount, o.created_at, p.name AS product_name
      FROM orders o LEFT JOIN products p ON p.id=o.product_id
      WHERE o.user_id=${user.id} ORDER BY o.created_at DESC LIMIT 3
    `,
  ])

  const stats = [
    { label:'Total Orders', value:ordersCount.count, icon:'📦', href:'/dashboard/orders', color:'#44aaff' },
    { label:'Downloads', value:downloadsCount.count, icon:'⬇️', href:'/dashboard/downloads', color:'#44dd88' },
    { label:'Reviews', value:reviewsCount.count, icon:'⭐', href:'/dashboard/reviews', color:'#f0a500' },
  ]
